...
export async function main() {
  ...
  const response = await client.path("/chat/completions").post({
    body: {
      messages: [
        { role: "system", content: "You are a front_end devloper." },
        { role: "user", content: [
            { type: "text", text: "devlope the front end of a web page as shown in the image. provide the code in html and css."},
            { type: "image_url", image_url: {
                url: getImageDataUrl("js-ai-build-a-thon/contoso_layout_sketch.jpg", "jpg"), details: "low"}}
          ]
        }
      ],
      model: modelName
